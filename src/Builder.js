Ext.define('CriteriaBuilder.Builder', {
    extend: 'Ext.Base',
    mixins: ['CriteriaBuilder.mixin.SQLParser', 'CriteriaBuilder.mixin.DSL'],
    config: {
        store: null
    },
    /**
    ...
    processCriteria: function(collection) {
        var me = this,
            ...
            matches=true,
            lookupCache = {},
            i, x, criterion, regex, row, compareMethod, rowValue, args,
            queryValue, matchingKeys, property, matchCount, instanceMatch,
        ...
        collection.each(function(row, index, len) {
            for(x in me.criteria) {
                ...
                // no scheme, just use simple match count (0 or 1)
                else {
                    matches = matchCount;
                }
